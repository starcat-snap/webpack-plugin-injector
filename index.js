...
class WebpackPluginInjector {
  ...
  constructor(filePath, webpackConfig, section, silent = false) {
    ...
    if (!sections.includes(section)) {
      throw new Error(
        `Section "${section}" is not a valid section. Available sections: ${sections.join(
          ", "
        )}`
      );
    }
  ...
  parsePluginDefinitions(pluginDefinitions) {
    ...
    Object.keys(pluginDefinitions).forEach((pluginName) => {
      ...
      sections.forEach((section) => {
        if (
          pluginDefinition[section] &&
          pluginDefinition[section].entryFilePath
        ) {
          const plugin = this.getPluginConfig(
            pluginName,
            pluginDefinition,
            section
          );
  ...
  getPluginConfig(pluginName, pluginDefinition, section) {
    ...
    const hasCustomStyleFiles = Object.prototype.hasOwnProperty.call(
      pluginDefinition[section],
      "styleFiles"
    )
      ? pluginDefinition[section].styleFiles.length > 0
      : false;
    const webpackConfigPath = !hasCustomWebpackConfig
      ? null
    ...
    if (pluginDefinition.administration && section === "administration") {
      assetPaths.push(
        join(basePath, pluginDefinition.administration.path, "../static")
      );
    }
    return {
      basePath,
  ...
  registerPluginsToWebpackConfig(plugins) {
    plugins.forEach((plugin) => {
      ...
      this.webpackConfig.entry[technicalName] =
        this.env === "development" && this.section === "administration"
      ...
      if (!plugin.hasCustomWebpackConfig) {
        this.warn(
          ...
          `Plugin "${name}" injected as a new entry point`
        );
        return;
      }
      ...
      if (typeof customConfigFn !== "function") {
        throw new Error(
          `Webpack config for plugin ${name} needs to be a function to extend the provided Webpack.`
        );
      }

      // Call custom webpack config function and merge the new config
      const modifiedWebpackConfig = customConfigFn(
        Object.assign(
          {},
          {
      ...
      this.warn(
        "Webpack Plugin Injector",
        `Plugin "${name}" injected with custom config`
      );
    });
